...
req('aes config_file lenc_tools')
...
module LEnc
  ...
  class Repo
    ...
    def create(repo_dir, key, enc_dir, original_names=false)
      ...
      config_file_path = File.join(repo_dir, LENC_REPO_FILENAME)
      if File.file?config_file_path
        raise ArgumentError, "Encryption repository already exists: #{config_file_path}"
      end
      @confFile = ConfigFile.new(config_file_path)
      ...
      if @verbosity >= 1
          puts("Creating encryption repository #{@confFile.path}")
      end
    ...
    def open(startDirectory = nil, password = nil)
      ...
      @repoBaseDir = File.dirname(cfile.path)
      ...
      if (!key_verifier)
        puts("Warning: no key verifier found in repository; assuming it's correct")
      else
    ...
    def self.findRepository(startDir)
      ...
      while true
        ...
        # Construct a ConfigFile object to determine if it already exists
        config_file_path = File.join(bp,LENC_REPO_FILENAME)
        if File.file?(config_file_path)
          return ConfigFile.new(config_file_path)
        end
    ...
    def encrypt_file(sourceFile, encryptFile)
      ...
      if File.directory?(encryptFile)
        ...
        if @verbosity >= 1
          puts "Encrypting file " + rel_path(sourceFile, @inputDir) \
              + " is overwriting existing directory: " + pth
        end

        if not @dryrun
          FileUtils.rm_rf(encryptFile)
        end
      end
    ...
    def encrypt_directory_contents(sourceDir, encryptDir)
      ...
      if File.file?(ignPath)
        f1 = FileUtils.read_text_file(ignPath)
      ...
      if !in_place? && !File.directory?(encryptDir)
        ...
        if not @dryrun
          if File.file?(encryptDir)
            ...
            if @verbosity >= 1
              printf("Encrypting directory is overwriting existing file: %s",pth)
            end
      ...
      dirc = LencTools.dir_entries(sourceDir)

      dirc.each do |f2|
        # Convert string to ASCII-8BIT encoding.
        f = LencTools.to_ascii8(f2)

        filePath = File.join(sourceDir,f)

        if File.symlink?(filePath)
          if @verbosity >= 0
            puts("Omitting symlink file '#{rel_path(filePath,@inputDir)}'")
          end
          next
        end
        ...
        if in_place?
          ...
          if f.start_with?(ENCRFILENAMEPREFIX)
            if @verbosity >= 0
              puts("(Omitting source file / dir with name that looks encrypted: #{f})")
            end
            next
          end
        end
      ...
      if !in_place?
        ...
        if File.directory?(encryptDir)
          dire = LencTools.dir_entries(encryptDir)
        else
          dire = []
        end
    ...
    def decrypt_directory_contents(encr_dir_path,decr_dir_name = nil)
      ...
      # Examine each file in source dir
      dirc = LencTools.dir_entries(encr_dir_path)
      ...
      dirc.each do |f2|
        ...
        if File.directory?(filePath)
          ...
          if !@orignames
            raise ArgumentError,"decrypted already exists: #{decrPath}" if File.exist?(decrPath)
            if not @dryrun
          ...
          begin
            ...
            if @verbosity >= 0
              puts("\nUnable to decrypt file: #{pth} (cause: #{e.message})\n")
            end
          end
        end
      end
    ...
    def recover(encryptDir, recoverDir)
      ...
      dirc = LencTools.dir_entries(encryptDir)
