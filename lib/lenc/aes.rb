...
=end
  ...
  class MyAES
    ...
    def initialize(encrypting, key, nonce=nil)
      ...
      key = LencTools.bytes_to_str(key)
    ...
    def setNonce(nonce=nil)
      if !nonce
          ...
          ni = LencTools.int_to_bytes(Time.now.usec)
          ni.concat(LencTools.int_to_bytes(@@nonceHelper))
          @@nonceHelper += 1
          nonce = LencTools.bytes_to_str(ni)
      end

      raise ArgumentError if !(nonce.is_a? String )
      LencTools.simple_str(nonce)


      nonce = LencTools.str_sized(nonce,NONCE_SIZE_SMALL)
...
#
def LencTools.str_sized(s, size, pad="\0")
